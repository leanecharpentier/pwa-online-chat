        ...
        const initCamera = async () => {
            ...
            try {
                ...
                while (!videoRef.current && attempts < 30) {
                    ...
                    attempts++;
                    console.log(`Tentative ${attempts}/30 - videoRef.current:`, videoRef.current);
                }
        ...
        try {
            ...
            if (savedPhotos) {
                ...
                const validPhotos = parsedPhotos.filter((photo: PhotoData) => {
                    ...
                    if (!isValid) {
                        console.warn("Photo invalide trouvée:", photo.id, "imageUrl (début):", photo.imageUrl.substring(0, 50));
                        console.warn("ImageUrl complète (premiers 200 caractères):", photo.imageUrl.substring(0, 200));
                    } else {
                        console.log("Photo valide:", photo.id, "Taille:", photo.imageUrl.length, "caractères");
                    }
                    return isValid;
                });
                ...
                const sortedPhotos = validPhotos.sort(
                    (a: PhotoData, b: PhotoData) =>
                        new Date(b.dateEmis).getTime() -
                        new Date(a.dateEmis).getTime()
                );
                ...
                {process.env.NODE_ENV === 'development' && (
                    <div className="p-4 bg-yellow-50 border-b">
                        <details className="text-xs">
                            <summary className="cursor-pointer font-semibold">Debug Info ({photos.length} photos)</summary>
                            <div className="mt-2 space-y-1">
                                {photos.map((photo) => (
                                    <div key={photo.id} className="bg-white p-2 rounded">
                                        <p>ID: {photo.id}</p>
                                        <p>URL length: {photo.imageUrl?.length || 0}</p>
                                        <p>URL start: {photo.imageUrl?.substring(0, 50) || 'N/A'}...</p>
                                        <p>Valid: {photo.imageUrl?.startsWith('data:image') ? '✅' : '❌'}</p>
                                    </div>
                                ))}
                            </div>
                        </details>
                                ...
                                <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 border border-gray-200 hover:border-blue-500 transition-colors relative">
                                    {imageErrors.has(photo.id) ? (
                                        <div className="w-full h-full flex items-center justify-center bg-gray-200">
                                            <div className="text-center p-4">
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        console.log("Tentative de rechargement de l'image:", photo.id);
                                                        console.log("Image URL (premiers 200 caractères):", photo.imageUrl.substring(0, 200));
                                                        setImageErrors(prev => {
                                        ...
                                        <img
                                            ...
                                            onError={(e) => {
                                                console.error("❌ Erreur de chargement de l'image:", photo.id);
                                                console.error("Image URL (début):", photo.imageUrl?.substring(0, 100));
                                                console.error("Image URL (longueur):", photo.imageUrl?.length);
                                                console.error("Image URL (fin):", photo.imageUrl?.substring(Math.max(0, photo.imageUrl.length - 50)));
                                                console.error("Élément img:", e.target);
                                                setImageErrors(prev => new Set(prev).add(photo.id));
                                            }}
                                            onLoad={(e) => {
                                                console.log("✅ Image chargée avec succès:", photo.id);
                ...
                >
                    ...
                    >
                        ...
                        <img
                            ...
                            onError={(e) => {
                                console.error("❌ Erreur dans la modal - Image ID:", selectedPhoto.id);
                                console.error("Image URL (début):", selectedPhoto.imageUrl?.substring(0, 100));
                                console.error("Image URL (longueur):", selectedPhoto.imageUrl?.length);
                            }}
                            onLoad={() => {
                                console.log("✅ Image chargée dans la modal:", selectedPhoto.id);
                            }}
                        />
